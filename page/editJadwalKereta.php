...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "SELECT * FROM keretaApi WHERE id = ?";
...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE keretaApi SET noSeri=?, nama=?, asal=?, tujuan=?, keberangkatan=?, kedatangan=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssi", $noSeri, $nama, $asal, $tujuan, $keberangkatan, $kedatangan, $id);
    if ($stmt->execute()) {
        header("Location: editjadwalKereta.php?msg=update_success&id=" . $id);
        exit;
    } else {
